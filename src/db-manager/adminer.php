...
if($automatic) $_POST=['auth'=>['driver'=>$driver,'server'=>$server,'username'=>'root','password'=>$config['mysqlPassword'],'db'=>$database]];
function adminer_object(): object {
    return new class extends Adminer\Adminer {
        ...
        function credentials(){global $server,$config;return [$server,$config['mysqlUser'],$config['mysqlPassword']];}
